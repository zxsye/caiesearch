    ...
    return new Promise((resolve, reject) => {
      ...
      if ((match = lQuery.match(/^!!index!([0-9a-f]+)$/))) {
        ...
        PastPaperDoc.find(parsedQuery.finder, {fileBlob: false, dir: false}).limit(71).then(rst => {
          ...
          // Enrich each doc with aggregated unique topics from doc.dir.dirs[i].topics
          // (this is where doLinkTopics.bin.js persists question-level Gemini tags).
          Promise.all(rst.map(doc => {
            ...
            return PastPaperDoc.findOne({_id: doc._id}, {dir: true}).then(fullDoc => {
              ...
              if (fullDoc && fullDoc.dir && Array.isArray(fullDoc.dir.dirs)) {
                for (let q of fullDoc.dir.dirs) {
                  if (Array.isArray(q.topics)) q.topics.forEach(t => topicsSet.add(t))
                }
              }
              let enriched = doc.toObject ? doc.toObject() : Object.assign({}, doc)
