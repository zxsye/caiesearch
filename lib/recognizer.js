...
module.exports = {
  ...
  noRectsLeftOf: function (rTree, rect) {
    if (typeof rect.y1 !== 'number') throw new Error('Number.isNumber rect.y1 === false')
    if (typeof rTree.all()[0].t !== 'string') throw new Error('rTree[].t should be the text for that rects.')
    let rectsLeftOf = rTree.search({
      minX: -Infinity, maxX: Math.min(rect.x1, rect.x2) - 0.1,
      minY: Math.min(rect.y1, rect.y2) - 4, maxY: Math.max(rect.y1, rect.y2) + 4
    })
    return rectsLeftOf.every(r => /^\s$/.test(r.t) || (r.minX + r.maxX) / 2 < 45)
  },
  regionBlank: function (rTree, region) {
    let rects = rTree.search(region)
    return rects.every(r => /^\s$/.test(r.t))
  },
  sortRects: function (idx) {
    let rects = idx.rects.map((r, i) => Object.assign({}, r, {t: idx.content[i]}))
    if (rects.length === 0) return {sortedRects: [], sortedContent: []}
  ...
  dir: function (idxes) {
    ...
    let doSearch = (fromII = 0) => {
      ...
      idxes.slice(fromII).forEach((idx, ii) => {
        ...
        for (let lastIndex = 0; (lastIndex = sortedContent.indexOf(search, lastIndex)) >= 0; lastIndex += 1) {
          let frontRect = sortedRects[lastIndex]
          let qRect = sortedRects[lastIndex]
          let tt = sortedContent.substring(lastIndex, lastIndex + 4);
          // Reject phantom matches whose digits span multiple y-buckets (e.g. a subscript
          // digit followed by the first digit of a real question number). The matched chars
          // must share a baseline and be horizontally contiguous to count as a single number.
          let matchRects = sortedRects.slice(lastIndex, lastIndex + search.length)
          let baselineY = matchRects[0].y2
          let onSameLine = matchRects.every(r => Math.abs(r.y2 - baselineY) < 0.5)
          let xContiguous = matchRects.every((r, k) => k === 0 || Math.abs(r.x1 - matchRects[k - 1].x2) < 2)
          if (!onSameLine || !xContiguous) continue
          // console.log(nextQuestionNum, frontRect, tt, xLimit, JSON.stringify(sortedContent.substr(lastIndex + nextQuestionNum.toString().length)));
          if (frontRect.x1 < xLimit && this.noRectsLeftOf(rTree, qRect)
            && (
                ...
                (() => {
                  let baseI = lastIndex + nextQuestionNum.toString().length;
                  let m = sortedContent.substr(baseI).match(/^(\([a-z]+\))*/);
                  if (!m) {
                    return false;
                  }
                  let lastCharRect = sortedRects[baseI + m[0].length - 1]
                  return this.regionBlank(rTree, {
                    minX: Math.max(lastCharRect.x1, lastCharRect.x2) + 0.1,
                    maxX: Math.max(lastCharRect.x1, lastCharRect.x2) + Math.abs(lastCharRect.x2 - lastCharRect.x1),
                    minY: Math.min(lastCharRect.y1, lastCharRect.y2),
                    maxY: Math.max(lastCharRect.y1, lastCharRect.y2),
                  });
                })()
              )) {
            let qT = ''
            ...
            xLimit = Math.min(xLimit, frontRect.x1 + Math.abs(frontRect.x2 - frontRect.x1) * 1.5)
            nextQuestionNum ++
  ...
  mcqMs: function (idxes) {
    ...
    const doSearch = (currentPage) => {
      ...
      for (let lastIndex = 0; (lastIndex = idx.content.indexOf(nextQuestionNum.toString(), lastIndex)) >= 0; lastIndex += nextQuestionNum.toString().length) {
        ...
        if ((
          /^\s$/.test(idx.content[lastIndex - 1]) && /^\s$/.test(idx.content[lastIndex + nextQuestionNum.toString().length])
        ) || (
          this.regionBlank(rTree, {
            minX: Math.min(thisRect.x1, thisRect.x2) - Math.abs(thisRect.x2 - thisRect.x1),
            maxX: Math.min(thisRect.x1, thisRect.x2) - 0.1,
            minY: Math.min(thisRect.y1, thisRect.y2),
            maxY: Math.max(thisRect.y1, thisRect.y2),
          }) && this.regionBlank(rTree, {
            minX: Math.max(lastRect.x1, lastRect.x2) + 0.1,
            maxX: Math.max(lastRect.x1, lastRect.x2) + Math.abs(lastRect.x2 - lastRect.x1),
            minY: Math.min(lastRect.y1, lastRect.y2),
            maxY: Math.max(lastRect.y1, lastRect.y2),
          })
        )) {
  ...
  noRectsRightOf: function (rTree, rect) {
    if (typeof rect.y1 !== 'number') throw new Error('Number.isNumber rect.y1 === false')
    if (typeof rTree.all()[0].t !== 'string') throw new Error('rTree[].t should be the text for that rects.')
    let upperY = Math.min(rect.y1, rect.y2)
    let lowerY = Math.max(rect.y1, rect.y2)
    let height = lowerY - upperY
    let rectsRightOf = rTree.search({
      minX: Math.max(rect.x1, rect.x2) + 0.5, maxX: Infinity,
  ...
  textInLineOf: function (rTree, rect) {
    if (typeof rect.y1 !== 'number') throw new Error('Number.isNumber rect.y1 === false')
    if (typeof rTree.all()[0].t !== 'string') throw new Error('rTree[].t should be the text for that rects.')
    let upperY = Math.min(rect.y1, rect.y2)
    let lowerY = Math.max(rect.y1, rect.y2)
  ...
  er: function (idxes) {
    ...
    for (idx of idxes) {
      ...
      for (let pPaperStr = 0; (searchReturn = idx.content.substr(pPaperStr).search(/\d\d\d\d\/\d\d/)) >= 0; pPaperStr += 8) {
        ...
        if (!this.noRectsRightOf(rTree, rects[rects.length - 1])) continue
  ...
  erDoOnePaper: function ({pv, idxes}) {
    ...
    for (let idx of idxes) {
      ...
      for (let lastIndex = 0; (lastIndex = sortedContent.indexOf(strQuestion, lastIndex)) >= 0; lastIndex += strQuestion.length) {
        ...
        let qRect = rectsQuestions[0]
        if (qRect.y1 > idx.maxY) break
        if (!this.noRectsLeftOf(rTree, qRect)) continue
        // We do not check for no RectsRightOf here. For reason, see 0470_s08_er.
        // Instead, we check that this is a new paragraph by testing whether there is space above.
        let toplineY = Math.min(qRect.y1, qRect.y2)
        let rectsAbove = rTree.search({
          minX: -Infinity, maxX: Infinity,
          minY: toplineY - 8, maxY: toplineY - 4
        })
        if (!rectsAbove.every(r => /^\s$/.test(r.t))) continue
        let textInThisLine = this.textInLineOf(rTree, qRect).split(':')[0].trim() // Question 1: Compulsory Data Response
